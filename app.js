(() => {
  ...
  function bind(id, fn){
    const el = $(id);
    if(el) el.onclick = fn;
    // if element doesn't exist, silently skip — never throws
  }
  ...
  function addExercise(val){
    ...
    div.innerHTML = `<input placeholder="Exercise name" value="${safeVal}">
  ...
  function startSelected(){
    ...
    idx = 0; round = 1; elapsed = 0; paused = false;
  ...
  function setState(s, dur, label){
    state = s; duration = dur; t = dur; paused = false;
    const bp = $("btnPause");
  ...
  function next(){
    switch(state){
      case "warmup":
        setState("work", cfg.work, exercises[0]); break;
      case "work":
        setState("rest", cfg.rest, "Rest"); break;
      case "rest":
        idx++;
        ...
        setState("work", cfg.work, exercises[idx]); break;
      case "cooldown":
        clearInterval(timer);
        state = "idle";
        showDone(); break;
  ...
  function updateUI(){
    const timerEl = $("timer"); if(timerEl) timerEl.textContent = t;
    const phase = $("phase");   if(phase)   phase.textContent = state.toUpperCase();
    const cur = $("current");
    ...
    const nextEl = $("next"); if(nextEl) nextEl.textContent = nextHint;
    const meta = $("meta");
    if(meta) meta.textContent = cfg ? `Round ${round} / ${cfg.rounds}` : "";
    const circle = $("progressCircle");
    if(circle && duration > 0){
      circle.style.strokeDashoffset = CIRC * (1 - t / duration);
